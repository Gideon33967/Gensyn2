  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const lossValue = (result.history.loss as number[])[0];
        addLog(`  Epoch ${i + 1} → loss ${lossValue.toFixed(4)}`);
            ...
            <div className="space-y-6">
              <div className="flex justify-between text-3xl">
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => {setRunning(false); setPaused(false);}} className="p-4 bg-red-600 rounded-xl">Stop</button>
                </div>
              </div>

              <div className="h-4 bg-gray-800 rounded-full overflow-hidden">
                <motion.div className="h-full bg-gradient-to-r from-purple-500 to-cyan-600"
                  animate={{ width: `${progress}%` }} />
              </div>

              <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 h-96 overflow-y-auto font-mono text-green-400 text-sm">
              ...
              <button onClick={() => navigator.clipboard.writeText(`I earned ${earnings.toFixed(2)} $SY on GenSyn Playground! https://your-new-url.vercel.app`)}
